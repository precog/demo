...
search_term: term to search in twitter
precog_host: ingestion host
precog_ingest_path: path where to store the data including the user id (it must be owned by the api key, i.e. using another api key + user & password is not supported yet
precog_apiKey: api key for ingestion (must have permissions in the ingest path)
alchemy_apiKey: Alchemy API key
minutes_between_requests: minutes between twitter api calls
